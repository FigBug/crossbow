...
- (void)windowDidLoad
{
    ...
    [self.window setFrameUsingName:@"browser"];
...
- (void)windowWillClose:(NSNotification *)notification
{
    ...
    [self.window saveFrameUsingName:@"browser"];
...
- (void)windowDidBecomeKey:(NSNotification *)notification
{
    ...
    {
        thumbnailThread = [[NSThread alloc] initWithTarget:self selector:@selector(thumbnailProc:) object:self.folderContents];
...
{
    return [NSArray arrayWithObjects:@"fol",@"nav",@"reload",@"slideshow",NSToolbarFlexibleSpaceItemIdentifier,nil];
}
...
- (NSArray*)makeToolbarItems
{
    ...
    tool = [SegmentedToolbarItem itemWithIdentifier:@"fol" label:@"Folders" paletteLabel:@"Folders" segments:2];
    [tool setSegment:0 imageName:NSImageNameAddTemplate longLabel:@"Add Folder" target:nil action:@selector(addFolder:)];
    [tool setSegment:1 imageName:NSImageNameRemoveTemplate longLabel:@"Remove Folder" target:nil action:@selector(removeFolder:)];
    [tool setupView];
    [array addObject:tool];

    popupFocus = tool.control;
...
- (BOOL)validateUserInterfaceItem:(id <NSValidatedUserInterfaceItem>)item
{
    ...
    if (sel == @selector(removeFolder:))
    {
        DirEntry* selectedDE = browserTree.selectedDirEntry;

        for (DirEntry* de in [DirEntry getDisplayRootItems])
            if ([de isEqual:selectedDE])
                return YES;
        return NO;
    }
    return YES;
...
- (IBAction)addFolder:(id)sender
{
    ...
    __weak BrowserTree* weakTree = browserTree;
    [panel beginSheetModalForWindow:self.window completionHandler:^(NSInteger result) {
        if (result == NSFileHandlingPanelOKButton)
        {
            ...
            for (NSURL* url in urls)
            {
                ...
                {
                    [DirEntry addDisplayRootItem:url.path];
                }
            }
            [weakTree devicesChanged:nil];
        }
    }];
    [self hideWelcome];
}
...
{
    [DirEntry removeDisplayRootItem:browserTree.selectedDirEntry.path];
    [browserTree devicesChanged:nil];
...
- (void)showWelcome
{
    if ([[DirEntry getDisplayRootItems] count] == 0)
    {
        ...
        if (!popover.isShown)
        {
            ...
            [popover showRelativeToRect:popupFocus.frame ofView:popupFocus preferredEdge:NSMinYEdge];
        }
